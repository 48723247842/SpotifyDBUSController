...
app = Sanic( name="Spotify DBus Controller Server" )

@app.route( "/" )
def hello( request ):
	return response.text( "You Found the Spotify DBus Controller Server!\n" )
...
def get_config( redis_connection ):
	try:
		try:
			config = redis_connection.get( "CONFIG.SPOTIFY_DBUS_CONTROLLER_SERVER" )
...
def run_server():
	try:
		...
		print( "Couldn't Start Spotify DBus Controller Server" )
		print( e )
		return False
...
try_run_block({
	"task_name": "Spotify DBus Controller Server" ,
	"number_of_tries": 5 ,
